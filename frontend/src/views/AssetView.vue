  ...
  try {
    ...
    if (storedUser) {
      try {
        ...
        console.log('AssetView: 从localStorage加载用户信息:', currentUser.value)
        console.log('AssetView: 用户权限:', currentUser.value?.is_superuser ? '管理员' : '普通用户')
        return
      } catch (e) {
        console.warn('AssetView: 解析localStorage中的用户信息失败:', e)
      }
    }
    
    // 如果localStorage中没有，从API获取
    console.log('AssetView: 从API获取用户信息...')
    currentUser.value = await authService.getCurrentUser()
    console.log('AssetView: 从API获取用户信息成功:', currentUser.value)
