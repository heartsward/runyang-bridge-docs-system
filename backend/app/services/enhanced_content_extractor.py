    ...
    def _post_process_ocr_text(self, raw_text: str) -> str:
        ...
        try:
            ...
            ocr_corrections = {
                ...
                # 政府文档常见误识别（根据实际OCR错误）
                '姚': '数', '益': '监', '督': '督',  # 保留督字，因为它可能是正确的
                '上': '工', 'f': '', '作': '作',  # f通常是识别错误，直接删除
            ...
            # 处理多个空格的情况
            text = re.sub(r'([\u4e00-\u9fff])\s{2,}([\u4e00-\u9fff])', r'\1\2', text)
            ...
            common_fixes = {
                ...
                # 政府文档常用词汇
                '组 织': '组织', '开 展': '开展', '监 督': '监督', '通 知': '通知',
                '政 府': '政府', '委 员': '委员', '单 位': '单位', '企 业': '企业',
                '事 业': '事业', '部 门': '部门', '机 关': '机关', '办 公': '办公',
                '实 施': '实施', '执 行': '执行', '落 实': '落实', '推 进': '推进',
                '完 善': '完善', '规 范': '规范', '标 准': '标准', '要 求': '要求',
                '措 施': '措施', '方 案': '方案', '计 划': '计划', '目 标': '目标',
                '任 务': '任务', '责 任': '责任', '职 责': '职责', '义 务': '义务',
                
                # 常见连接词和介词
                '根 据': '根据', '按 照': '按照', '依 据': '依据', '关 于': '关于',
                '对 于': '对于', '由 于': '由于', '为 了': '为了', '通 过': '通过',
                '采 用': '采用', '利 用': '利用', '运 用': '运用', '应 用': '应用',
                
                # 时间和地点
                '时 间': '时间', '地 点': '地点', '范 围': '范围', '区 域': '区域',
                '全 市': '全市', '各 区': '各区', '各 县': '各县', '全 国': '全国',
                '省 级': '省级', '市 级': '市级', '县 级': '县级', '乡 镇': '乡镇',
                
                # 专业术语（网络安全相关）
                '等 级': '等级', '保 护': '保护', '防 护': '防护', '安全 等级': '安全等级',
                '等级 保护': '等级保护', '常 态': '常态', '态 化': '态化', '常态 化': '常态化',
                '网络 安全': '网络安全', '数据 安全': '数据安全', '信息 安全': '信息安全'
            }
            ...
            specific_corrections = {
                # 基于实际OCR错误的特定修正
                '姚常': '数据', '益督': '监督', '上 f 作': '工作', '上f作': '工作',
                '于组': '关于组', '図内': '图内', '範围': '范围', '範圍': '范围',
                '怀': '患', '隐怀': '隐患', '等牢': '筑牢', '屏障': '屏障',
                
                # 网络安全专业术语的常见错误
                '等级保护单位': '等级保护单位', '网络強国': '网络强国', '強国': '强国',
                '資料': '资料', '數據': '数据', '網絡': '网络', '監督': '监督',
                
                # 政府文档标题的常见错误
                '関于': '关于', '於': '于', '発展': '发展', '実施': '实施'
            }
            
            # 按长度排序处理特定错误
            sorted_specific = sorted(specific_corrections.items(), key=lambda x: len(x[0]), reverse=True)
            for wrong, correct in sorted_specific:
                text = text.replace(wrong, correct)
            ...
            # 第八步：最终的中文字符间空格清理（再次执行，确保彻底）
            # 有时经过词汇替换后会产生新的空格
            prev_text = ""
            final_iteration = 0
            max_final_iterations = 5
            
            while prev_text != text and final_iteration < max_final_iterations:
                prev_text = text
                text = re.sub(r'([\u4e00-\u9fff])\s+([\u4e00-\u9fff])', r'\1\2', text)
                final_iteration += 1
