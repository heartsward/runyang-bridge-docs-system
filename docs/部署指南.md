...
> 注：早期版本依赖 LibreOffice 做内容提取，现已改为 anydoc（firecrawl-anydoc），无需安装 LibreOffice。
...
**Q: 文档内容无法提取**
A: 内容提取引擎为 anydoc（pip install firecrawl-anydoc），无需安装 LibreOffice。确认 `pip list | grep firecrawl-anydoc` 已安装即可。
