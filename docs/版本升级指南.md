...
检查配置文件是否有需要合并的变更（用 git diff 对比模板）：

```bash
# 对比后端/前端配置模板与依赖清单的变更
git diff HEAD~1 -- backend/.env.example frontend/.env.example \
    backend/requirements-windows.txt frontend/package.json
```
...
#### 手动比对
```bash
git diff HEAD~1 -- backend/.env.example frontend/.env.example backend/requirements-windows.txt frontend/package.json
```

#### 检测内容
- ✅ 后端配置文件 (.env vs .env.example)
- ✅ 前端配置文件 (.env vs .env.example)
- ✅ Python依赖文件 (requirements-windows.txt)
...
#### 手动重启服务
```bash
./stop-services.sh   # 停止服务（Windows: stop-services.bat）
./start-services.sh  # 重新启动（Windows: start-services.bat）
```
...
### start-services.bat / start-services.sh
- **用途**: 系统启动
- **功能**: 依赖检查（首次自动装）、服务启动（后端 :8002 + 前端 :5173）
- **适用**: 初始部署和服务重启
